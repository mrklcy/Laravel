...
@section('title', $service->name . ' - CLSU ADSO')
    ...
    <div class="absolute inset-0 flex items-center z-10 pointer-events-none">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            <div class="mb-4 pointer-events-auto">
                <a href="{{ route('services.index') }}" class="text-clsu-yellow hover:text-white text-sm">← Back to Services</a>
            </div>
            @if($service->office)
                <p class="text-clsu-yellow text-sm mb-2">{{ $service->office->acronym }}</p>
            @endif
            <h1 class="text-4xl font-bold mb-4 text-white drop-shadow-lg">{{ $service->name }}</h1>
        </div>
    </div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="bg-white rounded-lg shadow-md p-8 mb-8">
        ...
        @if($service->details)
            <div class="mb-6">
                <h3 class="text-xl font-semibold text-gray-900 mb-3">Details</h3>
                <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ $service->details }}</p>
            </div>
        @endif
        ...
        @if($service->requirements)
            <div class="mb-6 bg-gray-50 p-6 rounded-lg">
                ...
                <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ $service->requirements }}</p>
            </div>
        @endif

        @if($service->process)
            <div class="mb-6 bg-clsu-green/5 p-6 rounded-lg border-l-4 border-clsu-green">
                <h3 class="text-xl font-semibold text-gray-900 mb-3">Process</h3>
                <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ $service->process }}</p>
            </div>
        @endif
            ...
            <div class="bg-gradient-to-r from-clsu-green to-green-cobra rounded-lg p-6 text-white">
                ...
                <div class="space-y-2">
                    @if($service->contact_person)
                        <p><span class="font-semibold text-clsu-yellow">Contact Person:</span> {{ $service->contact_person }}</p>
                    @endif
                    @if($service->contact_email)
                        <p><span class="font-semibold text-clsu-yellow">Email:</span> {{ $service->contact_email }}</p>
                    @endif
                    @if($service->contact_phone)
                        <p><span class="font-semibold text-clsu-yellow">Phone:</span> {{ $service->contact_phone }}</p>
                    @endif
                </div>
            </div>
        @endif
    ...
    <div>
        <h2 class="text-2xl font-bold text-green-cobra mb-6">Related Services</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($relatedServices as $related)
                <a href="{{ route('services.show', $related->slug) }}" class="group">
                    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all border border-gray-200 hover:border-clsu-green">
                        <div class="bg-gradient-to-br from-clsu-green to-green-cobra px-6 py-4">
                            <h3 class="text-lg font-bold text-white group-hover:text-clsu-yellow transition-colors">{{ $related->name }}</h3>
                        </div>
                        <div class="p-6">
