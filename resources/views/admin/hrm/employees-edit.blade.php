...
@section('crumb','Edit Employee')

@section('content')

<div class="mb-4">
  <h1 class="h3 fw-bold mb-1">Edit Employee</h1>
  <p class="text-muted mb-0">Update employee information for {{ $employee->first_name }} {{ $employee->last_name }}</p>
</div>
  ...
  <div class="col-lg-8">
    <div class="card rounded-4 border">
      <div class="card-body p-4">
        <form action="{{ route('admin.hrm.employees.update', $employee->id) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <input type="date" name="date_hired" class="form-control @error('date_hired') is-invalid @enderror" value="{{ old('date_hired', $employee->date_hired ? $employee->date_hired->format('Y-m-d') : '') }}">
              @error('date_hired')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-md-6">
              ...
              <select name="employment_status" class="form-select @error('employment_status') is-invalid @enderror" required>
                ...
                <option value="casual" {{ old('employment_status', $employee->employment_status) == 'casual' ? 'selected' : '' }}>Casual</option>
                <option value="job_order" {{ old('employment_status', $employee->employment_status) == 'job_order' ? 'selected' : '' }}>Job Order</option>
              </select>
            ...
            <div class="col-12">
              <div class="alert alert-info rounded-4">
                <strong>Note:</strong> This employee is in the <strong>Human Resources</strong> department.
              </div>
            </div>
          </div>
      ...
      <div class="card-body p-4">
        ...
        <div class="small">
          <div class="mb-2">
            ...
            <span class="fw-bold">{{ $employee->created_at->format('M d, Y') }}</span>
          </div>
          <div class="mb-2">
            <span class="text-muted">Last Updated:</span>
            <span class="fw-bold">{{ $employee->updated_at->format('M d, Y') }}</span>
          </div>
          <div>
