...
<head>
  <meta charset="UTF-8">
  <title>@yield('title','HRMO Admin') - CLSU</title>
  ...
  <style>
    @php
      $office = \App\Models\Office::where('code', 'HRMO')->first();
  ...
  <aside class="sidebar p-3" id="sidebar">
    <div class="d-flex align-items-center gap-3 p-3 border rounded-4 mb-3 brand">
      ...
      <div class="label-text">
        <div class="fw-bold">HRMO Panel</div>
        <div class="text-muted small brand-sub">@yield('crumb','HRMO Dashboard')</div>
      </div>
    </div>
    ...
    <nav class="nav flex-column gap-1">
      ...
      <a class="nav-link {{ request()->routeIs('admin.hrm.employees') ? 'active' : '' }}"
         href="{{ route('admin.hrm.employees') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('admin.hrm.analytics') ? 'active' : '' }}"
         href="{{ route('admin.hrm.analytics') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('admin.hrm.reports') ? 'active' : '' }}"
         href="{{ route('admin.hrm.reports') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('admin.hrm.inquiries*') ? 'active' : '' }}"
         href="{{ route('admin.hrm.inquiries') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('admin.hrm.settings') ? 'active' : '' }}"
         href="{{ route('admin.hrm.settings') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
  ...
  <main class="main-area">
    ...
    <nav class="navbar bg-white border-bottom px-4">
      <div class="d-flex align-items-center gap-2">
        ...
        <span class="text-muted">@yield('crumb','HRMO')</span>
      </div>
        ...
        <div class="dropdown">
          <button class="btn d-flex align-items-center gap-2 border rounded-4 px-3 py-2 bg-white" type="button" data-bs-toggle="dropdown">
            <div class="rounded-4 fw-bold d-flex align-items-center justify-content-center"
              ...
              {{ strtoupper(substr(Auth::guard('admin')->user()->name ?? 'H', 0, 1)) }}
            </div>
            <div class="small text-start">
              <div class="fw-bold">{{ Auth::guard('admin')->user()->name ?? 'HRMO Admin' }}</div>
              <div class="text-muted">{{ ucfirst(Auth::guard('admin')->user()->role ?? 'HRMO Administrator') }}</div>
            </div>
          </button>
