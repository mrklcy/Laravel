...
<div class="d-flex justify-content-between align-items-start mb-3">
  <div class="d-flex align-items-center gap-3">
    ...
    <div>
      ...
      <div class="text-muted">Update event details</div>
    </div>
  </div>
</div>
        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST">
          ...
          <div class="mb-3">
            ...
            <select class="form-select rounded-3 @error('office_id') is-invalid @enderror" 
              ...
              @foreach($offices as $office)
                <option value="{{ $office->id }}" {{ old('office_id', $event->office_id) == $office->id ? 'selected' : '' }}>
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control rounded-3 @error('content') is-invalid @enderror" 
                      id="content" name="content" rows="6">{{ old('content', $event->content) }}</textarea>
            @error('content')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control rounded-3 @error('location') is-invalid @enderror" 
                   id="location" name="location" value="{{ old('location', $event->location) }}" placeholder="e.g., CLSU Auditorium">
            @error('location')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-md-6">
              ...
              <input type="datetime-local" class="form-control rounded-3 @error('event_end_date') is-invalid @enderror" 
                     id="event_end_date" name="event_end_date" value="{{ old('event_end_date', $event->event_end_date ? $event->event_end_date->format('Y-m-d\TH:i') : '') }}">
    ...
    <div class="card rounded-4 border bg-light">
      <div class="card-body">
        ...
        <ul class="small text-muted mb-0">
          <li class="mb-2"><strong>Created:</strong> {{ $event->created_at->format('M d, Y h:i A') }}</li>
          <li class="mb-2"><strong>Updated:</strong> {{ $event->updated_at->format('M d, Y h:i A') }}</li>
