...
<div class="d-flex justify-content-between align-items-start mb-3">
  <div class="d-flex align-items-center gap-3">
    ...
    <div>
      <h1 class="h3 fw-bold mb-1">Edit Office</h1>
      <div class="text-muted">Update {{ $office->name }}</div>
    </div>
  </div>
</div>
        ...
        <form action="{{ route('admin.offices.update', $office) }}" method="POST">
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control rounded-3 @error('acronym') is-invalid @enderror" 
                     id="acronym" name="acronym" value="{{ old('acronym', $office->acronym) }}" required>
              @error('acronym')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control rounded-3 @error('overview') is-invalid @enderror" 
                      id="overview" name="overview" rows="4">{{ old('overview', $office->overview) }}</textarea>
          ...
          <div class="row">
              ...
              <select class="form-select rounded-3 @error('parent_id') is-invalid @enderror" 
                ...
                @foreach($parentOffices as $parent)
                  <option value="{{ $parent->id }}" 
                    {{ old('parent_id', $office->parent_id) == $parent->id ? 'selected' : '' }}>
            ...
            <div class="col-md-6 mb-3">
              ...
              <input type="number" class="form-control rounded-3 @error('order') is-invalid @enderror" 
                     id="order" name="order" value="{{ old('order', $office->order) }}" min="0" required>
      ...
      <div class="card-body">
        ...
        <div class="small">
          <div class="mb-2">
            ...
            <span class="fw-bold">{{ $office->created_at->format('M d, Y') }}</span>
          </div>
          <div class="mb-2">
            <span class="text-muted">Last Updated:</span>
            <span class="fw-bold">{{ $office->updated_at->format('M d, Y') }}</span>
          </div>
          @if($office->children()->count() > 0)
          <div class="mb-2">
            <span class="text-muted">Sub-offices:</span>
            <span class="fw-bold">{{ $office->children()->count() }}</span>
          </div>
          @endif
        </div>
      </div>
    ...
    <div class="card rounded-4 border bg-light mt-3">
      <div class="card-body">
        ...
        <ul class="small text-muted mb-0">
          <li class="mb-2">Changing the code may break existing links</li>
          <li class="mb-2">Deactivating an office hides it from public view</li>
          <li>Adjust display order to reorganize office listings</li>
        </ul>
      </div>
    </div>
  </div>
