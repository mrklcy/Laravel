...
<head>
  <meta charset="UTF-8">
  <title>@yield('title','PSO Panel') - CLSU</title>
  ...
  <style>
    @php
      $office = \App\Models\Office::where('code', 'PSO')->first();
  ...
  <aside class="sidebar p-3" id="sidebar">
    <div class="d-flex align-items-center gap-3 p-3 border rounded-4 mb-3 brand">
      ...
      <div class="label-text">
        <div class="fw-bold">PSO Panel</div>
        <div class="text-muted small brand-sub">@yield('crumb','Property & Supply')</div>
      </div>
    </div>
    ...
    <nav class="nav flex-column gap-1">
      ...
      <!-- PSO Dashboard -->
      <a class="nav-link {{ request()->routeIs('pso.dashboard') ? 'active' : '' }}"
         href="{{ route('pso.dashboard') }}">
      ...
      <div class="mt-3 mb-2 px-3">
        <small class="text-muted fw-bold label-text">INVENTORY</small>
      </div>

      <a class="nav-link {{ request()->routeIs('pso.projects*') ? 'active' : '' }}"
         href="{{ route('pso.projects') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
          <path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>
        </svg>
        <span class="label-text">Property Records</span>
      </a>

       <a class="nav-link {{ request()->routeIs('pso.strategic-plans*') ? 'active' : '' }}"
         href="{{ route('pso.strategic-plans') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
        ...
        <span class="label-text">Supply Management</span>
      </a>
      ...
      <a class="nav-link {{ request()->routeIs('pso.analytics') ? 'active' : '' }}"
         href="{{ route('pso.analytics') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('pso.reports') ? 'active' : '' }}"
         href="{{ route('pso.reports') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('pso.inquiries*') ? 'active' : '' }}"
         href="{{ route('pso.inquiries') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('pso.settings') ? 'active' : '' }}"
         href="{{ route('pso.settings') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
  ...
  <main class="main-area">
    ...
    <nav class="navbar bg-white border-bottom px-4">
      <div class="d-flex align-items-center gap-2">
        ...
        <span class="text-muted">@yield('crumb','Property & Supply')</span>
      </div>
        ...
        <div class="dropdown">
          <button class="btn d-flex align-items-center gap-2 border rounded-4 px-3 py-2 bg-white" type="button" data-bs-toggle="dropdown">
            <div class="rounded-4 fw-bold d-flex align-items-center justify-content-center"
              ...
              {{ strtoupper(substr(Auth::guard('admin')->user()->name ?? 'S', 0, 1)) }}
            </div>
            <div class="small text-start">
              <div class="fw-bold">{{ Auth::guard('admin')->user()->name ?? 'PSO Admin' }}</div>
              <div class="text-muted">{{ ucfirst(Auth::guard('admin')->user()->role ?? 'Property & Supply Admin') }}</div>
            </div>
          </button>
