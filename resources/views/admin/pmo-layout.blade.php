...
<head>
  <meta charset="UTF-8">
  <title>@yield('title','PMO Admin') - CLSU</title>
  ...
  <style>
    @php
      $office = \App\Models\Office::where('code', 'PMO')->first();
  ...
  <aside class="sidebar p-3" id="sidebar">
    <div class="d-flex align-items-center gap-3 p-3 border rounded-4 mb-3 brand">
      ...
      <div class="label-text">
        <div class="fw-bold">PMO Panel</div>
        <div class="text-muted small brand-sub">@yield('crumb','PMO Dashboard')</div>
      </div>
    </div>
    ...
    <nav class="nav flex-column gap-1">
      ...
      <!-- PMO Dashboard -->
      <a class="nav-link {{ request()->routeIs('pmo.dashboard') ? 'active' : '' }}"
         href="{{ route('pmo.dashboard') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
          <path d="M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z"/>
        </svg>
        <span class="label-text">PMO Dashboard</span>
      </a>

      <!-- Facilities Section -->
      <div class="mt-3 mb-2 px-3">
        <small class="text-muted fw-bold label-text">PROCUREMENT</small>
      </div>

      <a class="nav-link {{ request()->routeIs('pmo.buildings') ? 'active' : '' }}"
         href="{{ route('pmo.buildings') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 7V3H2v18h20V7H12zM6 19H4v-2h2v2zm0-4H4v-2h2v2zm0-4H4V9h2v2zm0-4H4V5h2v2zm4 12H8v-2h2v2zm0-4H8v-2h2v2zm0-4H8V9h2v2zm0-4H8V5h2v2zm10 12h-8v-2h2v-2h-2v-2h2v-2h-2V9h8v10zm-2-8h-2v2h2v-2zm0 4h-2v2h2v-2z"/>
        </svg>
        <span class="label-text">Purchase Requests</span>
      </a>

      <a class="nav-link {{ request()->routeIs('pmo.equipment') ? 'active' : '' }}"
         href="{{ route('pmo.equipment') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
          <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92 1.61 0 2.92-1.31 2.92-2.92s-1.31-2.92-2.92-2.92z"/>
        </svg>
        <span class="label-text">Suppliers & Items</span>
      </a>

      <!-- Services Section -->
      <div class="mt-3 mb-2 px-3">
        <small class="text-muted fw-bold label-text">OPERATIONS</small>
      </div>

      <a class="nav-link {{ request()->routeIs('pmo.maintenance') ? 'active' : '' }}"
         href="{{ route('pmo.maintenance') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
          <path d="M22.7 19l-9.1-9.1c.9-2.3.4-5-1.5-6.9-2-2-5-2.4-7.4-1.3L9 6 6 9 1.6 4.7C.4 7.1.9 10.1 2.9 12.1c1.9 1.9 4.6 2.4 6.9 1.5l9.1 9.1c.4.4 1 .4 1.4 0l2.3-2.3c.5-.4.5-1.1.1-1.4z"/>
        </svg>
        <span class="label-text">Order Tracking</span>
      </a>

      <a class="nav-link {{ request()->routeIs('pmo.reservations') ? 'active' : '' }}"
         href="{{ route('pmo.reservations') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
          <path d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1h-2zm3 18H5V8h14v11z"/>
        </svg>
        <span class="label-text">Bid Management</span>
      </a>
      ...
      <a class="nav-link {{ request()->routeIs('pmo.analytics') ? 'active' : '' }}"
         href="{{ route('pmo.analytics') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('pmo.reports') ? 'active' : '' }}"
         href="{{ route('pmo.reports') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('pmo.inquiries*') ? 'active' : '' }}"
         href="{{ route('pmo.inquiries') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a class="nav-link {{ request()->routeIs('pmo.settings') ? 'active' : '' }}"
         href="{{ route('pmo.settings') }}">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
  ...
  <main class="main-area">
    ...
    <nav class="navbar bg-white border-bottom px-4">
      <div class="d-flex align-items-center gap-2">
        ...
        <span class="text-muted">@yield('crumb','PMO')</span>
      </div>
        ...
        <div class="dropdown">
          <button class="btn d-flex align-items-center gap-2 border rounded-4 px-3 py-2 bg-white" type="button" data-bs-toggle="dropdown">
            <div class="rounded-4 fw-bold d-flex align-items-center justify-content-center"
              ...
              {{ strtoupper(substr(Auth::guard('admin')->user()->name ?? 'G', 0, 1)) }}
            </div>
            <div class="small text-start">
              <div class="fw-bold">{{ Auth::guard('admin')->user()->name ?? 'PMO Admin' }}</div>
              <div class="text-muted">{{ ucfirst(Auth::guard('admin')->user()->role ?? 'PMO Administrator') }}</div>
            </div>
          </button>
